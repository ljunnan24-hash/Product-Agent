...
CODE_EXECUTOR_SANDBOX=auto # 可选：auto / process / docker；docker 严格模式不可用时会阻断
...
- Durable Worker Replay 已有第五段：新增 `durable-worker-replay.ts`，`POST /api/worker-queue` 支持 `replay`，可读取 durable input payload 并重放 `web_search` / `web_fetch` / `code_execute` / `evidence_extract` / `judge` / `model_report` worker，结果会写入 `.taste-data/artifacts` 或对应 runtime artifact，并回填 durable record 的 `artifactIds`、`outputArtifactRefs`、状态、latency 和 replay metrics。`runtime-resume` 会优先查找同 trace 下关联 workerRunId 的 durable record，并尝试执行 durable replay；如果 replay 返回结构化搜索结果或网页正文，会合并回 `WebResearchSummary`，重算 Evidence Brief，并重跑 Judge、Report Composer、证据绑定和报告质检；如果 replay 返回 `code_execute`，会更新代码执行 trace/artifact，并在已有实验结果时刷新实验计算证据和 Evidence Brief，不误报为网页证据刷新；如果 replay 返回 `judge` 或 `model_report`，会刷新 Judge verdict、报告、证据绑定和质量审计。当前下游刷新粒度仍是分析记录级重算，后续会继续缩小到 task graph 局部节点。
...
- Resume Executor 已有第三段：`POST /api/analyses/:id/runtime-resume` 会保存恢复请求；Judge / Report Composer 末端节点可自动重放，刷新 Judge verdict、报告、证据绑定和质量审计。搜索/抓取 task node 可根据 taskNodeId 批量 replay durable `web_search` / `web_fetch` records，并在 replay 产出结构化结果后刷新 Evidence Brief、Judge、Report 和质量审计。`evidence_extract` / `loop:*:evidence_extract` 现在可复用当前 `WebResearchSummary` 重建 `evidence_cards` artifact 和 handoff，再刷新 Evidence Brief、Judge、Report 和质量审计；它不重新联网，所以上游搜索/抓取变化仍应先恢复对应 task node。
- Replay Impact Ledger 已有第一段：`AgentRuntimeResumeRequest.impact` 会记录恢复范围、source task、replayed task/worker/tool、durable queue record、downstream task、recomputed 派生产物、artifact refs 和 notes。报告页的 Resume 请求会显示 scope/source/downstream/recomputed，让用户看到本次恢复是 worker、task、evidence_extract 还是 terminal 级，哪些下游节点被刷新。当前仍是“影响范围可审计 + 分析记录级刷新”，下一段继续把实际计算粒度缩小到 task graph 局部节点。
- Resume Impact TaskGraph 回写已有第一段：保存最终 resume request 时，会把 `impact` 同步回 `AgentRuntimeTrace.taskGraph`。被 replay 的 source/replayed 节点会记录 `lastResumeRequestId`、`lastReplayScope`、`lastReplayStatus`、durable record 数、artifact 数、recomputed 摘要；downstream 节点会记录由哪个恢复请求刷新，并在报告页 task graph 卡片显示 `resume <scope>`。当前是 task graph 状态/审计层局部化，实际 Evidence Brief/Judge/Report 计算仍是分析记录级。
